...
function DashBoard() {
  ...
  const chartConfig = {
    ...
    options: {
      ...
      title: {
        text: user_info.user_type === "lu_admin" 
      ...
      <div className='flex flex-col h-full w-full'>
        <div className='flex items-center gap-3 basis-1/4 px-2 overflow-x-auto mx-6 my-4 pb-2'>
          <div className='h-4/5 w-full'>
            <SimpleCard body={`${dashboard_data?.total_ghge.toFixed(2)}`} header={user_info.user_type === "lu_admin" ? 'Total LU GHGe' : 'Total GHGe'}  
          ...
          <SimpleCard 
            body={
              user_info.user_type === "lu_admin" 
                  ...
                  : user_info.user_type === "s-admin" 
                      ? `${(dashboard_data?.total_LU_surveyors || 0) + (dashboard_data?.total_surveryor || 0)}`
                      : `${dashboard_data?.total_surveryor}`} 
                      header={user_info.user_type === "lu_admin" ? 'Total LU Surveyor' : 'Total Surveyor'} 
                      icon={ user_info.user_type === "lu_admin" 
                        ? <img src={lu_img} className="h-6 w-6 rounded-lg" />
          ...
          <div className='h-4/5 w-full'>
            ...
            body={
              user_info.user_type === "lu_admin" 
                  ...
                  : user_info.user_type === "s-admin" 
                      ? `${(dashboard_data?.total_LU_admins || 0) + (dashboard_data?.total_LGU_admins || 0)}`
                      : `${dashboard_data?.total_LGU_admins}`
}             header={user_info.user_type === "lu_admin" ? 'Total LU Admin' : 'Total LGU Admin'} 
              icon={ user_info.user_type === "lu_admin" 
       ...
       <div className=" md:w-1/3 h-1/4 text-xs bg-white ml-4 pt-2 rounded-lg shadow-md mr-4">
          ...
          <Typography className='bg-darkgreen font-bold text-base mb-4 rounded-lg py-2 -mt-2 text-center' color='white'>
            ...
            <div className="mb-4 px-4 h-20">
              <SimpleCard
              body={`${dashboard_data?.table_data.mobileCombustionGHGe.reduce((acc, val) => acc + val.emission.ghge, 0).toFixed(2)}`}
              header="Mobile Combustion"
              icon={<GlobeAsiaAustraliaIcon className="h-6 w-6" />}
              child_card={<BarChart chart_icon={<GlobeAsiaAustraliaIcon className="h-6 w-6" />} chart_label='Mobile Combustion' 
              chart_meaning = {
                user_info.user_type === "s-admin" 
                  ...
                  : user_info.user_type === "lu_admin" 
                    ...
                    series={[chartConfig.series[0] as any]} isLoading = {isLoading}/>}
              isLoading={isLoading}
              />
            </div>
                ...
                chart_meaning = {
                  user_info.user_type === "s-admin" 
                    ...
                    : user_info.user_type === "lu_admin" 
                      ? "GHGe in Laguna University." 
                      : `GHGe per Brgy in ${user_info.municipality_name}.`}
                      series={[chartConfig.series[1] as any]} isLoading = {isLoading}/>}
                isLoading={isLoading}
              />
            </div>
                ...
                chart_meaning = {
                  user_info.user_type === "s-admin" 
                    ...
                    : user_info.user_type === "lu_admin" 
                      ? "GHGe in Laguna University." 
                      : `GHGe per Brgy in ${user_info.municipality_name}.`}
                series={[chartConfig.series[2] as any]} isLoading = {isLoading}/>}
                isLoading={isLoading}
              />
            </div>
                ...
                chart_meaning = {
                  user_info.user_type === "s-admin" 
                    ...
                    : user_info.user_type === "lu_admin" 
                      ? "GHGe in Laguna University." 
                      : `GHGe per Brgy in ${user_info.municipality_name}.`}
                series={[chartConfig.series[3] as any]} isLoading = {isLoading}/>}
                isLoading={isLoading}
              />
            </div>
                ...
                chart_meaning = {
                  user_info.user_type === "s-admin" 
                    ...
                    : user_info.user_type === "lu_admin" 
                      ? "GHGe in Laguna University." 
                      : `GHGe per Brgy in ${user_info.municipality_name}.`}
                series={[chartConfig.series[4] as any]} isLoading = {isLoading}/>}
                isLoading={isLoading}
              />
            </div>
