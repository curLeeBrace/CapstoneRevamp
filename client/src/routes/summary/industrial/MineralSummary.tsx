    ...
    return(
        ...
        <div className="w-full flex justify-center mx-8 mb-4">
                ...
                <div className="my-2 px-2">
                        {
                            donutState ?
                                <DonutChart labels={donutState.labels} series={donutState.series} title="Mineral" chart_meaning={`Overall collected data in ${user_info.user_type === "s-admin" ? "Laguna Province." : user_info.user_type === "lgu_admin" ? ` ${user_info.municipality_name}.` : "Brgy."}`}/>
                                :<SimpleCard body={"No Available Data"} header="" icon={<ExclamationTriangleIcon className="h-full w-full"/>}/>
                        }
                </div>
